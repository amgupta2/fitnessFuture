      ...
      <div className="p-6">
        <div className="max-w-7xl mx-auto">
          <div className="animate-pulse">
            <div className="h-8 bg-[var(--surface-3)] rounded w-48 mb-6"></div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-24 bg-[var(--surface-3)] rounded"></div>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-[var(--surface-2)] border border-[var(--border)] rounded-lg p-8 text-center">
            <p className="text-[var(--text-2)]">Please log in to view your analytics</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="p-4 md:p-6 pb-24">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
          ...
          {/* Time Range Selector */}
          <div className="flex gap-2 bg-[var(--surface-2)] p-1 rounded-lg border border-[var(--border)]">
            {[
              { value: 7, label: "7D" },
              { value: 30, label: "30D" },
              { value: 90, label: "90D" },
        ...
        {/* Volume Over Time */}
        <div className="bg-[var(--surface-2)] rounded-lg p-4 md:p-6 border border-[var(--border)] mb-6">
        ...
        <div className="bg-[var(--surface-2)] rounded-lg p-4 md:p-6 border border-[var(--border)] mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-3">
            ...
            <select
              ...
              className="bg-[var(--surface-3)] border border-[var(--border)] rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-[rgba(107,142,232,0.4)]"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Personal Records */}
          <div className="bg-[var(--surface-2)] rounded-lg p-4 md:p-6 border border-[var(--border)]">
            <h2 className="text-xl font-semibold mb-4">Recent PRs</h2>
            {prs ? (
              <PRList records={prs} />
            ) : (
              <div className="text-center py-8 text-[var(--text-2)]">Loading...</div>
            )}
          </div>
        ...
        {/* Metrics Explanation */}
        <div className="mt-6 bg-[var(--surface-2)]/50 rounded-lg p-4 border border-[var(--border)]">
          <h3 className="text-sm font-semibold text-[var(--text-2)] mb-2">
            About These Metrics
          </h3>
          <div className="text-xs text-[var(--text-2)] space-y-1">
            <p>
              <strong className="text-[var(--text-2)]">Volume:</strong> Total weight
              lifted (weight × reps × sets)
            </p>
            <p>
              <strong className="text-[var(--text-2)]">Est. 1RM:</strong> Estimated
              one-rep max using the Epley formula: weight × (1 + reps/30)
            </p>
            <p>
              <strong className="text-[var(--text-2)]">Plateau:</strong> Less than 2.5%
